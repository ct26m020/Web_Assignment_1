<script setup lang="ts">
import type { Todo } from '../types/todo';

const props = defineProps({
    todo: {
        type: Object as () => Todo,
        required: true,
    },
});

</script>

<template>
    <div class="todo-item">
        <input type="checkbox" :checked="props.todo.done" @change="$emit('toggle-todo', props.todo.id)" />
        <span :class="{ done: props.todo.done }">{{ props.todo.text }}</span>
        <button @click="$emit('delete-todo', props.todo.id)">
            <svg width="800px" height="800px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M10 11V17" stroke="red" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                <path d="M14 11V17" stroke="red" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                <path d="M4 7H20" stroke="red" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                <path d="M6 7H12H18V18C18 19.6569 16.6569 21 15 21H9C7.34315 21 6 19.6569 6 18V7Z" stroke="red"
                    stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                <path d="M9 5C9 3.89543 9.89543 3 11 3H13C14.1046 3 15 3.89543 15 5V7H9V5Z" stroke="red"
                    stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
            </svg>
        </button>
    </div>
</template>
